...
export function MessagesClient({
  ...
  useEffect(() => {
    async function loadConversations() {
      ...
      if (error) {
        ...
        setConversations((data ?? []) as unknown as ConversationNode[]);
      }
  ...
  const filteredConversations = conversations.filter((c) => {
    ...
    const otherParticipant =
      c.participants.find(
        (p) => p.profile_id !== userId || (activeTab === "company" && p.organization_id)
      ) || c.participants[0];
    ...
    <div className="flex h-[calc(100vh-8rem)] min-h-[500px] overflow-hidden bg-[#1F1F1F] border border-white/10 shadow-sm rounded-[10px] text-white">
      ...
      <div className="w-[320px] md:w-[380px] border-r border-white/10 flex flex-col bg-[#1F1F1F]">
        <div className="p-6 border-b border-white/10">
          <h2 className="text-2xl font-semibold tracking-tight text-[var(--brand)] mb-4">
            {t("inbox")}
          </h2>
          <Tabs
            value={activeTab}
        ...
        <ScrollArea className="flex-1 w-full">
          <div className="flex flex-col gap-2 p-2 w-full">
            {isLoadingConversations ? (
              ...
              <p className="text-center text-muted-foreground p-4">{t("noConversations")}</p>
            ) : (
              filteredConversations.map((conv) => {
                ...
                const title =
                  otherParticipant?.organization?.name ||
                  otherParticipant?.profile?.full_name ||
                  t("unknown");
                const avatarLabel = title.charAt(0).toUpperCase();
                const avatarUrl =
                  otherParticipant?.organization?.logo_url || otherParticipant?.profile?.avatar_url;

                const lastMsg = conv.last_message?.[0];
                const isUnread =
                  lastMsg && !lastMsg.is_read && lastMsg.sender_profile_id !== userId;
                ...
                return (
                  ...
                  >
                    <Avatar className="h-10 w-10 border border-white/10 shadow-sm mt-1">
                      ...
                      <AvatarFallback className="bg-[#121212] text-gray-400">
                        {avatarLabel}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 overflow-hidden">
                      <div className="flex items-baseline justify-between mb-1">
                        <p
                          className={`text-sm truncate ${isUnread ? "font-bold text-white" : "font-semibold text-gray-300"}`}
                        >
                          {title}
                        </p>
                        {lastMsg && (
                      ...
                      <p
                        className={`text-sm leading-snug truncate ${isUnread ? "text-white font-medium" : "text-gray-400"}`}
                      >
        ...
        {activeConversationId ? (
          <>
            ...
            <div className="px-6 py-5 border-b border-white/10 flex flex-shrink-0 items-center justify-between bg-[#1F1F1F] z-10 sticky top-0 shadow-sm">
              <h3 className="font-semibold text-xl tracking-tight">{t("conversation")}</h3>
            </div>
              ...
              {isLoadingMessages ? (
                ...
                <div className="flex flex-col gap-4">
                  {messages.map((msg) => {
                    ...
                    const title = msg.organization?.name || msg.profile?.full_name || t("unknown");
                    const avatarLabel = title.charAt(0).toUpperCase();
                      ...
                      >
                        {!isMe && (
                          <Avatar className="h-8 w-8 shrink-0 mt-1">
                            <AvatarImage src={avatarUrl || ""} />
                            <AvatarFallback className="text-xs bg-[#1F1F1F] text-gray-400 border border-white/10">
                              {avatarLabel}
                            </AvatarFallback>
                          </Avatar>
                        )}
                        <div className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                          ...
                          <div
                            className={`px-4 py-2 text-sm shadow-sm ${isMe ? "bg-[var(--brand)] text-[#121212] rounded-2xl rounded-tr-sm" : "bg-[#1F1F1F] text-white border border-white/10 rounded-2xl rounded-tl-sm"}`}
                          >
            ...
            <div className="p-4 bg-[#1F1F1F] border-t border-white/10 flex-shrink-0">
              <form
                ...
                className="flex items-center gap-3 w-full bg-[#121212] border border-white/10 rounded-xl p-1 shadow-sm focus-within:border-[var(--brand)]/50 transition-all"
              >
                <Input
                ...
                <Button
                  ...
                  className="rounded-lg h-12 w-12 shrink-0 mr-1 bg-white/10 hover:bg-white/20 text-white border-0"
                >
                  {isSending ? (
