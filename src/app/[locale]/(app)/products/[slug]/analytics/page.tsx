...
import {
  getProductAnalytics,
  getProductScanners,
} from "@/features/products/server/analytics-actions";
import { ProductScannersList } from "@/features/products/components/product-scanners-list";
...
export default async function ProductAnalyticsPage({ params, searchParams }: Props) {
  ...
  const [analytics, scanners] = await Promise.all([
    getProductAnalytics(product.id, days),
    getProductScanners(product.id),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <ProductScannersList data={scanners} />
    </div>
  );
}
