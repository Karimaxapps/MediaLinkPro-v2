...
export async function createProduct(
  ...
  const gate = await checkOrgPlanLimit(validated.data.organization_id, "list_product");
  if (!gate.allowed) {
    return {
...
export async function addCommunityVideo(
  productId: string,
  title: string,
  url: string
): Promise<ActionState> {
...
export async function toggleResourceUpvote(resourceId: string): Promise<ActionState> {
  ...
  if (existingUpvote) {
    ...
    // Add upvote
    const { error: insertError } = await supabase.from("product_resource_upvotes").insert({
      user_id: user.id,
...
export async function joinProductExperts(
  productId: string,
  ...
  if (error) {
    if (error.code === "23505") {
      // Unique violation
      return {
        error: "You are already registered as a user/expert for this product.",
        success: false,
      };
    }
...
export async function addProductResource(data: {
  ...
  const { error } = await supabase.from("product_resources").insert(resourceData);
...
export async function updateProductResource(
  resourceId: string,
  data: {
    title?: string;
    url?: string;
  ...
  const { error } = await supabase.from("product_resources").update(data).eq("id", resourceId);
...
export async function deleteProductResource(resourceId: string): Promise<ActionState> {
  ...
  const { error } = await supabase.from("product_resources").delete().eq("id", resourceId);
...
export async function removeProductTrainingVideo(
  productId: string,
  videoUrl: string
): Promise<ActionState> {
  const cookieStore = await cookies();
...
export async function getProductScans(productId: string) {
  ...
  const scannerIds = scans.filter((s) => s.scanner_id).map((s) => s.scanner_id);
