...
function Header() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold tracking-tight text-white mb-1">{t("myApplications")}</h1>
      <p className="text-sm text-gray-400">{t("myAppsSubtitle")}</p>
    </div>
  );
}
...
function ApplicationRow({ application }: { application: JobApplication }) {
  ...
  const statusLabel = t(`appStatuses.${status}`);
    ...
    <div className="rounded-xl border border-white/10 bg-white/5 p-5 space-y-4">
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            {job && org ? (
              ...
              <div className="font-semibold text-white">{t("unknownRole")}</div>
            )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
        ...
        {application.interview_scheduled_at && (
          <div className="flex items-center gap-2 rounded-md border border-[#8b5cf6]/40 bg-[#8b5cf6]/10 px-3 py-2 text-gray-200">
            ...
            {t("interviewColon", {
              when: format(new Date(application.interview_scheduled_at), "PPpp"),
            })}
          </div>
        )}
      </div>
      ...
      {application.interview_location && (
        <div className="text-sm text-gray-400">
          <span className="text-gray-500">{t("locationColon")}</span>{" "}
          {application.interview_location}
        </div>
      )}
      {application.interview_notes && (
        <div className="text-sm text-gray-400 whitespace-pre-line">
          <span className="text-gray-500">{t("instructionsColon")}</span>{" "}
          {application.interview_notes}
        </div>
      )}
      ...
      {status !== "withdrawn" && status !== "rejected" && status !== "accepted" && (
        <div className="flex justify-end">
          ...
          >
            {isPending ? t("withdrawing") : t("withdrawApplication")}
          </Button>
        </div>
      )}
    </div>
