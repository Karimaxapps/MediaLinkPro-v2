...
export async function createEvent(eventData: {
  ...
  const gate = await checkOrgPlanLimit(eventData.organization_id, "publish_event");
  if (!gate.allowed) {
    return {
      success: false,
      error: blockedFeatureMessage("publish_event", gate.requiredPlan!),
    };
  }
...
export async function getMyEventInterest(eventId: string): Promise<EventInterest | null> {
  const cookieStore = await cookies();
  const supabase = createClient(cookieStore);
...
export async function setEventInterest(
  ...
  const { error } = await supabase
    .from("event_interests" as never)
    .upsert({ event_id: eventId, user_id: user.id, interest }, { onConflict: "event_id,user_id" });
