...
export function ProductDiscussions({
  ...
  const tr = useTranslations("discussions");
  ...
  const handleCreate = () => {
    startTransition(async () => {
      ...
      if (!result.success) {
        toast.error(result.error ?? tr("failedCreate"));
        return;
      }
      toast.success(tr("discussionStarted"));
      setNewTitle("");
      setNewBody("");
  ...
  const handleReply = () => {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        toast.error(result.error ?? tr("failedReply"));
        return;
      }
      setReplyBody("");
    ...
    return (
      <div className="space-y-4">
        ...
        >
          ...
          {tr("backToDiscussions")}
        </button>
        <div>
          <h3 className="text-xl font-bold text-white">{activeThread.title}</h3>
          <p className="text-xs text-gray-500 mt-1">
            {tr("startedBy", {
              name: activeThread.author?.full_name ?? activeThread.author?.username ?? tr("user"),
            })}
        ...
        <div className="space-y-3">
          {posts.length === 0 ? (
            <div className="text-sm text-gray-500 py-4">{tr("loading")}</div>
          ) : (
            posts.map((p) => (
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-medium text-white">
                      {p.author?.full_name ?? p.author?.username ?? tr("unknown")}
                    </div>
    ...
    <div className="space-y-4">
      ...
      {threads === null ? (
        ...
        <div className="rounded-lg border border-white/10 bg-white/5 p-8 text-center">
          ...
          <p className="text-sm text-gray-400">{tr("noDiscussions")}</p>
          {canPost && <p className="text-xs text-gray-500 mt-1">{tr("beFirst")}</p>}
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                    <span className="flex items-center gap-1">
                      ...
                      {t.author?.full_name ?? t.author?.username ?? tr("unknown")}
                    </span>
                    <span className="flex items-center gap-1">
                      <MessageSquare className="h-3 w-3" />
                      {tr("replyCount", { count: t.post_count })}
                    </span>
                    {t.updated_at && (
                      <span>
