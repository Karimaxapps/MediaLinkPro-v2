...
// Re-export types only. emailTemplates is re-exported from a non-"use server"
// barrel below so callers can import both from one module if desired.
...
export async function notify(input: NotifyInput): Promise<{ success: boolean; error?: string }> {
    ...
    try {
        ...
        const categoryPref = (prefs as Record<string, unknown> | null)?.[TYPE_TO_PREF[input.type]] ?? true;
