...
type Props = {
  post: BlogPost;
  /** Where the back link points: "/blog" (public) or "/blog/feed" (app shell). */
  backHref: string;
};

/**
 * Full blog article body, shared between the public route (/blog/[slug])
 * and the in-app route (/blog/feed/[slug]). Shows the complete article for
 * signed-in users and a gated preview with a sign-in CTA for guests.
 */
export async function ArticleView({ post, backHref }: Props) {
