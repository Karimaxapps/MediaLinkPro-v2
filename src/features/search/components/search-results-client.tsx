...
const tabs: { key: Tab; labelKey: string; icon: React.ElementType }[] = [
  { key: "all", labelKey: "all", icon: SearchX },
  { key: "products", labelKey: "products", icon: Package },
  { key: "companies", labelKey: "companies", icon: Building2 },
  { key: "people", labelKey: "people", icon: User },
  { key: "experts", labelKey: "experts", icon: Award },
];
  ...
  if (!initialQuery || initialQuery.length < 2) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h3 className="text-lg font-semibold text-white mb-1">{t("startSearching")}</h3>
        <p className="text-sm text-gray-400 max-w-sm">{t("startSearchingDesc")}</p>
      </div>
    );
  }
  ...
  if (totalCount === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-sm text-gray-400 max-w-sm">{t("noResultsDesc")}</p>
      </div>
    );
  }
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {product.organization && (
                    <p className="text-sm text-gray-400">
                      {t("by", { name: product.organization.name })}
                    </p>
                  )}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {profile.job_title && (
                    <p className="text-sm text-gray-400 truncate">
                      {profile.company
                        ? t("jobAtCompany", { title: profile.job_title, company: profile.company })
                        : profile.job_title}
                    </p>
                  )}
                  {profile.bio && (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="font-medium text-white truncate">
                    {expert.profile?.full_name || t("expert")}
                  </h3>
                  {expert.headline && (
                    <p className="text-sm text-gray-400 truncate">{expert.headline}</p>
                  )}
